...
y<-ggplot(data=subset(df19, df19$V3 == "V"), aes(x=((V4 + 249999)/1e6), y=(prop)))+
geom_vline(xintercept = 30, colour = "black", linewidth = 0.6, linetype=5)+
geom_vline(xintercept = 45, colour = "black", linewidth = 0.6, linetype=5)+
geom_line(aes(colour=test), linewidth=0.8)+
scale_colour_viridis_d()+
facet_grid(.~V3, scales = "free_x", switch="x")+
labs(y="SNP Proportion", x= "Genomic position (Mb)")+
...
patch <- x/y
patch2 <- patch + plot_annotation(tag_levels = 'A') + plot_layout(axis_titles = 'collect', axes = 'collect', ncol=1) & theme(plot.tag = element_text(size = 20), plot.tag.position = c(0.05, 1))
patch2


ggsave("HcSLPS_SNP_PROP_CMH_Chr_500kb_point_patch_allcomparisons_romnumerals_simplerYaxis_updated10-09-25.tiff", patch2, device=tiff, width=2250, height=2250, units="px", dpi=320)
```
<img width="2250" height="2250" alt="image" src="https://github.com/user-attachments/assets/6d6f8011-2d0a-4b52-8352-a114e338f951" />
